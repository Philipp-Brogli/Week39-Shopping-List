...
public class MainActivity extends Activity implements AddProductDialogFragment.DialogListener{
    ...
    public void queryData() {
        //cursor = db.rawQuery("SELECT _id, product, count, prize FROM highscores ORDER BY score DESC", null);
        // get data with query
        String[] resultColumns = new String[]{"_id","product","count","prize"};
        cursor = db.query(DATABASE_TABLE,resultColumns,null,null,null,null,"score DESC",null);

        // add data to adapter
        ListAdapter adapter = new SimpleCursorAdapter(this,
                R.layout.list_item, cursor,
                new String[] {"product", "count","prize"},      // from
                new int[] {R.id.product, R.id.count, R.id.prize}    // to
                ,0);  // flags
    ...
    @Override
    public void onDialogPositiveClick(DialogFragment dialog, String product, int count, int prize) {
        ContentValues values=new ContentValues(2);
        values.put("product", product);
        values.put("count", count);
        values.put("prize", prize);
        db.insert("newproduct", null, values);
        // get data again
        queryData();
    }
